...
{
	ros::init(argc,argv, "pose_decision");
	ros::NodeHandle n;
    ...
    //	ros::Publisher pub_pass = n.advertise<tf_ros_detection::PosePass>("/posepass",1);
    	...
    	ros::Publisher pub_result = n.advertise<sensor_msgs::Image>("/result",1);
	...
	private_nh_.getParam("delay",delay);	
	
	
	//wait till we receive images
    	while (ros::ok() && (is_first_image || is_first_depth || is_first_pose)) {
        	ros::spinOnce();
        	r.sleep();
    	}
...
// image = imread("/home/ubuntu/Downloads/i4.jpeg");
 ...
 //     cout << "p5" << endl;          
                ...
                if (danger){
                	...
                	msg1.data = 2;//gpioWrite(17, 0); gpioWrite(25, 1); 
                	}
                else{
                	//cout << "SOCIAL DISTANCE MAINTAINED" << endl;
                	msg1.data = 1;//gpioWrite(17, 1); gpioWrite(25, 0); 
                }
   //   cout << "p6" << endl;    
                sensor_msgs::Image result_show = *( cv_bridge::CvImage(header_org,"rgb8",frame.clone()).toImageMsg() );
                pub_result.publish(result_show);

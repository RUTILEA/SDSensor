		...
		waitKey(1);
	        ...
	        vector<vector<float>> face_location;
                	...
                	if (person_check(rec)==1)
			    {
			    	...
			    	putText(frame, "face" + std::to_string((int)((faces[i].score.data)*100))+" "+std::to_string(d_mean), Point(rec.x, rec.y+5) ,FONT_HERSHEY_SIMPLEX,0.7, cv::Scalar(0, 255, 255), 1, 8, 0);
			    }
                }
                ...
                for (int i=0;i<rois.size();i++)
                {
                	for (int j=0;j<faces.size();j++)
                	{
                		...
                		sensor_msgs::RegionOfInterest ROI_j = faces[i].roi;
